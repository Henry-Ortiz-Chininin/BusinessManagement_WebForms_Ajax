...
function drilldownmenu(setting){
	this.sublevelarrow={src: setting.ruta + '/Images/right.gif', width:'8px', top:'3px', left:'6px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow = setting.ruta + '/Images/right.gif' //full URL to image separating breadcrumb links (if it's defined)
	this.loadingimage = setting.ruta + '/Images/loader.gif' //full URL to 'loading' image, if menu is loaded via Ajax
	this.homecrumbtext = setting.homecrumbtext //Top level crumb text
	this.titlelength=35 //length of parent LI text to extract from to use as crumb titles
	this.backarrow = setting.ruta + '/Images/leftarrow.gif' //full URL to image added in front of back LI 
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.$uls.each(function(i){ //loop through each UL
		...
		if (i==0){ //if topmost UL
			$('<li class="backcontroltitle">Portal</li>').prependTo($thisul).click(function(e){e.preventDefault()})
			...
			$('<li class="backcontrol"><img src="'+thisdrill.backarrow+'" style="border-width:0" /> Atras</li>') //add back LI item
